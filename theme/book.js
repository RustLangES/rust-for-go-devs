...
(function codeSnippets() {
    ...
    function run_go_code(code_block) {
        ...
        if (text.trim() === '') {
            result_block.innerText = 'No code to run';
        ...
        fetch_with_timeout('https://test.cors.workers.dev/?https://go.dev/_/compile?backend=', {
            ...
            .then(response => {
                if (response.Errors) {
                    result_block.innerText = 'Playground Communication: ' + response.Errors;
                    return;
                }
                if (response.Events?.length > 0) {
                    result_block.innerText = response.Events.map(event => event.Message).join('\n');
                    result_block.classList.remove('result-no-output');
                }
            })
    ...
    Array.from(document.querySelectorAll('pre code.language-go')).forEach(function (block) {
        ...
        runCodeButton.hidden = false; // Always show for Go code blocks
        buttons.insertBefore(runCodeButton, buttons.firstChild);
        runCodeButton.addEventListener('click', function (e) {
            console.log("Run code button clicked", e);
            // For Go code blocks, we use a different function
